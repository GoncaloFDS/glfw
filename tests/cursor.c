...
int main(void)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (track_cursor)
        {
            ...
            vertices[0][0] = 0.5f;
            vertices[0][1] = (float) fb_height - floorf(cursor_y * scale) - 1.f + 0.5f;
            vertices[1][0] = (float) fb_width + 0.5f;
            vertices[1][1] = (float) fb_height - floorf(cursor_y * scale) - 1.f + 0.5f;
            vertices[2][0] = floorf(cursor_x * scale) + 0.5f;
            vertices[2][1] = 0.5f;
            vertices[3][0] = floorf(cursor_x * scale) + 0.5f;
